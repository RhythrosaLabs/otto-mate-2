...
const STARTER_TEMPLATES = [
  {
    ...
    label: "Landing Page",
    icon: Layout,
    prompt: "Build a beautiful modern SaaS landing page with a hero section, features grid, pricing table with 3 tiers, testimonials carousel, and a CTA footer. Use a dark theme with teal accents.",
  },
  {
    id: "dashboard",
    label: "Dashboard",
    icon: Monitor,
    prompt: "Build an analytics dashboard with a sidebar navigation, top stats cards showing KPIs, a line chart for revenue trends, a bar chart for user growth, a recent activity feed, and a data table with sorting and filtering.",
  },
  {
    id: "ecommerce",
    label: "E-Commerce",
    icon: Palette,
    prompt: "Build an e-commerce product page with a product image gallery, product details, size/color selectors, add-to-cart button, related products section, and customer reviews. Make it look like a premium fashion brand.",
  },
  {
    id: "portfolio",
    label: "Portfolio",
    icon: FileText,
    prompt: "Build a creative portfolio website for a designer with a minimal aesthetic, project gallery with hover effects, about section, skills visualization, contact form, and smooth scroll animations.",
  },
  {
    id: "todo",
    label: "Todo App",
    icon: Check,
    prompt: "Build a full-featured todo app with categories, due dates, priority levels, drag-and-drop reordering, local storage persistence, search/filter, dark mode, and smooth animations. Make it beautiful.",
  },
  {
    id: "chat",
    label: "Chat Interface",
    icon: MessageSquare,
    prompt: "Build a modern chat application UI with a conversation list sidebar, message bubbles with timestamps, message input with emoji picker, online status indicators, typing animations, and a beautiful dark theme.",
  },
];
...
function PreviewPanel({
  ...
  const previewHtml = useMemo(() => {
    ...
    let html = project.files[entry];

    // Gather all CSS and JS files
    const cssFiles = Object.entries(project.files).filter(
      ([p]) => p.endsWith(".css")
    );
    const jsFiles = Object.entries(project.files).filter(
      ([p]) => p.endsWith(".js") && !p.endsWith(".config.js") && p !== entry
    );

    if (!html) {
      // No HTML entry — construct one from scratch
      return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
<body>
  ...
  ${jsFiles.map(([, c]) => `<script>\n${c}\n<\/script>`).join("\n")}
    ...
    // Track which files have been inlined via their references
    const inlinedFiles = new Set<string>();

    // Replace <link> tags referencing local CSS files with inline <style>
    for (const [filePath, content] of Object.entries(project.files)) {
      if (!filePath.endsWith(".css")) continue;
      // Match various href patterns: "styles.css", "./styles.css", "/styles.css", "css/styles.css"
    ...
    // Replace <script src="..."> tags referencing local JS files with inline <script>
    for (const [filePath, content] of Object.entries(project.files)) {
    ...
    // Inject any CSS/JS files that weren't referenced in the HTML
    ...
    if (unreferencedCSS.length > 0) {
      const cssBlock = unreferencedCSS
        .map(([, c]) => `<style>\n${c}\n</style>`)
        .join("\n");
      // Inject before </head> or at the top of the document
    ...
    if (unreferencedJS.length > 0) {
      const jsBlock = unreferencedJS
        .map(([, c]) => `<script>\n${c}\n<\/script>`)
        .join("\n");
      // Inject before </body> or at the end of the document
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-2 px-3 py-1 rounded-md bg-pplx-bg/80 border border-pplx-border/50 min-w-[200px]">
            ...
            <span className="text-[11px] text-pplx-muted truncate">
              {project?.title ? `localhost:3000 — ${project.title}` : "localhost:3000"}
            </span>
          </div>
        </div>
...
export function AppBuilderClient() {
  ...
  const generateApp = useCallback(
    async (prompt: string, mode: "generate" | "modify" | "fix" | "explain" = "generate") => {
      ...
      addLog("command", mode === "modify" ? "Modifying app..." : mode === "fix" ? "Fixing bug..." : "Generating app...");
      ...
      try {
        ...
        const res = await fetch("/api/app-builder", {
          ...
          body: JSON.stringify({
            prompt,
            currentFiles: mode !== "generate" ? project?.files : undefined,
        ...
        // Parse the artifact response to extract project files
        try {
          // Try Bolt-style artifact format first, then fall back to JSON
          console.log("[AppBuilder] Parsing response, length:", fullText.length);
          console.log("[AppBuilder] Contains <boltArtifact>:", fullText.includes("<boltArtifact"));
          console.log("[AppBuilder] Contains <boltAction>:", fullText.includes("<boltAction"));

          const artifactResult = parseArtifactResponse(fullText);
          console.log("[AppBuilder] Artifact parse result:", artifactResult ? `${Object.keys(artifactResult.files).length} files` : "null");
